...
export function InstallTabs() {
  ...
  return (
    <div className="border border-pitch-black rounded-2xl overflow-hidden bg-light-linen shadow-[2px_2px_0px_#000] w-full max-w-sm sm:max-w-md">
      {/* Tab row — scrollable on mobile so labels never get clipped */}
      <div className="flex border-b border-pitch-black overflow-x-auto scrollbar-hide">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setActive(t.id)}
            className={`shrink-0 flex-1 min-w-[52px] py-2 text-[11px] font-mono font-medium tracking-wide transition-colors ${
              active === t.id
